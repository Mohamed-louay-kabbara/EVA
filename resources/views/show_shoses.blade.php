...
<head>
    ...
    <!-- site metas -->
    <title>{{ trans('main_trans.shoses') }}</title>
...
<body class="main-layout">
    ...
    <div class="collection_text">{{ trans('main_trans.shoses') }}</div>
    <div class="collection_section layout_padding">
        <div class="container">
            ...
            <strong>
                @if (App::getLocale() == 'en')
                    <p class="consectetur_text text-center" style="font-size:20px">
                    {{ trans('main_trans.show_shose_title')}}
                        <span style="font-size:30px;color:#6218a5">
                        {{ $name_category->name_en }}
                        </span>
                        <p>
                    @else
                    <p class="consectetur_text text-center" style="font-size:20px">
                ...
                @foreach ($shoses as $s)
                    <div class="col-sm-4">
                        <div class="best_shoes">
                            <p class="best_text" style="font-size:40px;color:red">{{ $s->model }}</p>
                            <div class="shoes_icon"><img src="{{ asset('imgs/' . $s->image) }}"></div>
                            <div class="star_text">
                                <div class="left_part">
                                    <ul>
                                        <div style="background:{{ $s->color }}; width:6rem;height:3rem;"></div>
                                        <br>
                                        <h1 class="text-right" style="font-weight:bold">{{ trans('main_trans.size_shoes') }} {{ $s->size }}</h1>
                                        <h1 class="text-right" style="font-weight:bold">{{ trans('main_trans.count_shoes') }} {{ $s->cont }}</h1>
                                    </ul>
                                </div>
                                <div class="right_part">
                                    <div class="shoes_price"><span style="color:green;">{{ $s->price }} $</span>
                                    </div>
                                </div>
                                <div class="buy_now_bt">
                                    <div class="col-md-12">
                                        <button type="button" class="btn btn"
                                            style="width:120px; background-color:#ff4e5b; color:#fff; font-weight:bold; font-size:20px" 
                                            data-bs-toggle="modal"
                                            data-bs-target="#insert{{ $s->id }}">{{ trans('main_trans.buy_shoes') }}</button>
                                        <div class="modal fade" id="insert{{ $s->id }}" tabindex="-1"
                                            aria-labelledby="exampleModalLabel2" aria-hidden="true">
                                            <div class="modal-dialog">
                                                <div class="modal-content">
                                                    <div class="modal-header">
                                                        <h1 class="modal-title fs-5" id="exampleModalLabel2">{{ trans('main_trans.buy_shoes') }}</h1>
                                                    ...
                                                    <div class="modal-body">
                                                        <form action="{{ route('orders.store') }}" method="post">
                                                            @csrf
                                                            <h3> {{trans('main_trans.show_shose_alert')}}
                                                            </h3>
                                                            <input type="text" name="address" class="form-control"
                                                                id="recipient-name">
                                                            <div class="btn-group" role="group"
                                                                aria-label="Basic example">
                                                                <input type="hidden" name="shose_id"
                                                                    value="{{ $s->id }}">
                                                                <a href="{{ route('addorder', $s->id) }}"
                                                                    class="btn btn outline m-1"
                                                                    style="background-color:#e83e8c">{{ trans('main_trans.normal') }}</a>

                                                                <button type="submit" class="btn  btn-outline m-1"
                                                                    style="background-color: #6610f2">{{ trans('main_trans.dele') }}</button>
